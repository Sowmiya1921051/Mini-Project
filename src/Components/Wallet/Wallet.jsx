...
const Wallets = () => {
  return (
    <div className="text-white mt-20 pl-24 pr-24 p-4">
      ...
      <h1 className="text-white text-7xl font-oxanium font-medium capitalize break-words text-center mt-5">wallets we support</h1>

      <div className="flex space-x-4 items-center justify-between mt-14">
        {walletData.map(({ name, image }, index) => (
          <div key={index} className="h-full rounded-lg bg-white bg-opacity-10 p-8 cursor-pointer group hover:scale-110 transition-transform duration-300 ease-in-out">
            <div className="w-40 h-40 rounded-full bg-white bg-opacity-10 p-4 mb-8 relative">
              <img className="m-4 w-24 h-24 object-cover rounded-full group-hover:scale-110 transition-transform duration-300 ease-in-out" src={image} alt={name} />
            </div>
